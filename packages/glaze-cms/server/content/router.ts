 ...
 * its single-segment endpoints (e.g. `get-session`). `setup` is where the first admin is claimed,
 * `pending-approvals` is where a schema change is decided, and `entities` is the content-model
 * descriptor.
 */
