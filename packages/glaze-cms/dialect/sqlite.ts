...
/**
 * Opens a SQLite database using Node's built-in `node:sqlite` driver.
 * @param path - The SQLite file path.
 * @returns A live database handle.
 */
async function createNodeSqlite(path: string): Promise<DatabaseHandle> {
	...
	// `node:sqlite` `.all()` executes and returns rows for a reader, and executes + returns `[]` for a
	// writer/DDL/PRAGMA — so one path serves every statement (no reader/columns routing needed).
	const runRaw: RawExecutor = (sql) =>
 ...
 * (`bun:sqlite` on Bun, `node:sqlite` on Node) via the runtime seam.
 *
...
/** The SQLite dialect adapter (bun:sqlite on Bun, node:sqlite on Node). */
